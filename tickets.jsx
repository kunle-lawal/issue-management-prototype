// Tickets page: list of submitted issue reports with detail sidebar
const { useState: useStateTickets, useEffect: useEffectTickets } = React;
...
// ---- Affected bookings summary (inline in sidebar) ----
...
// ---- Affected bookings detail view (replaces sidebar) ----
function AffectedBookingsDetail({ ticket, onBack, onClose }) {
  const [confirmed, setConfirmed] = useStateTickets(() => new Set());
  const [toast, setToast]         = useStateTickets(null);
...
function TicketSidebar({ ticket, onClose, onStatusChange }) {
  const [showBookings, setShowBookings] = useStateTickets(false);
...
// ---- Tickets page ----
function TicketsPage({ onNavigate, reports, onReport }) {
  const [selectedId, setSelectedId] = useStateTickets(null);
  const [buildingFilter, setBuildingFilter] = useStateTickets('sf');
  const [floorFilter, setFloorFilter]       = useStateTickets('Floor 4');
  const [resourceFilter, setResourceFilter] = useStateTickets('all');
  const [sortBy, setSortBy] = useStateTickets('default');
  const [viewMode, setViewMode] = useStateTickets('table');
  const [mapHoveredId, setMapHoveredId] = useStateTickets(null);
  const [mapDetailId, setMapDetailId]   = useStateTickets(null);
