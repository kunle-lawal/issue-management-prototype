...
export function Builder({ building, initialConfig, onCancel, onSave }) {
  const [config, setConfig] = useState(() => initialConfig || buildInitialConfig());
  const [activeId, setActiveId] = useState(DATA.RESOURCES[0].id);
  const [previewOpen, setPreviewOpen] = useState(false);
  const [reviewed, setReviewed] = useState(() => new Set([DATA.RESOURCES[0].id]));
