...
function IssueReportForm({ resource, resourceConfig, building, floor, onSubmit, onBack, onClose, onViewOnMap }) {
  ...
  const [amenitySelected, setAmenitySelected] = useState(new Set());
  const [unusableAnswer, setUnusableAnswer] = useState(null);
...
export function MapPage({ onNavigate, configs, reports, onReport }) {
  const [buildingId, setBuildingId] = useState('sf');
  const [floor, setFloor] = useState('Floor 4');
